struct binary_protocol
{
    ...
    unsigned int (*match)(struct vfs_protocol *protocol, struct vfs_backend *backend, unsigned int id);
    unsigned int (*find_symbol)(struct vfs_protocol *protocol, struct vfs_backend *backend, unsigned int id, unsigned int count, const char *symbol);
    unsigned int (*copy_program)(struct vfs_protocol *protocol, struct vfs_backend *backend, unsigned int id);
    unsigned int (*relocate)(struct vfs_protocol *protocol, struct vfs_backend *backend, unsigned int id, unsigned int address);
