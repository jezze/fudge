...
static struct multi_task *create_task()
{
    ...
    for (i = 1; i < MULTI_ENTRIES; i++)
    {
        ...
        mmu_map_memory(&tasks[i].directory, &tasks[i].tables[0], TASKADDRESS_PHYSICAL + i * TASKADDRESS_SIZE, TASKADDRESS_VIRTUAL, TASKADDRESS_SIZE, MMU_TABLE_FLAG_PRESENT | MMU_TABLE_FLAG_WRITEABLE | MMU_TABLE_FLAG_USERMODE, MMU_PAGE_FLAG_PRESENT | MMU_PAGE_FLAG_WRITEABLE | MMU_PAGE_FLAG_USERMODE);
        mmu_map_memory(&tasks[i].directory, &tasks[i].tables[1], STACKADDRESS_PHYSICAL + i * STACKADDRESS_SIZE, STACKADDRESS_VIRTUAL - STACKADDRESS_SIZE, STACKADDRESS_SIZE, MMU_TABLE_FLAG_PRESENT | MMU_TABLE_FLAG_WRITEABLE | MMU_TABLE_FLAG_USERMODE, MMU_PAGE_FLAG_PRESENT | MMU_PAGE_FLAG_WRITEABLE | MMU_PAGE_FLAG_USERMODE);
