...
void virtio_setqueue(unsigned short io, unsigned short index, struct virtio_queue *vq, unsigned int address)
{
    ...
    vq->buffers = (struct virtio_buffer *)(unsigned long)vq->address;
    vq->bufferssize = 16 * vq->size;
    vq->availablehead = (struct virtio_availablehead *)(unsigned long)(vq->address + vq->bufferssize);
    vq->availablering = (struct virtio_availablering *)(unsigned long)(vq->availablehead + 1);
    vq->availablesize = 6 + 2 * vq->size;
    vq->usedhead = (struct virtio_usedhead *)(unsigned long)(vq->address + pagealign(vq->bufferssize + vq->availablesize));
    vq->usedring = (struct virtio_usedring *)(unsigned long)(vq->usedhead + 1);
...
void virtio_setrx(unsigned short io, struct virtio_queue *vq, void *buffer)
{
    ...
    vq->buffers[bi].address = (unsigned long)buffer;
