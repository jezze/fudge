...
void mouse_notifymove(struct mouse_interface *interface, char relx, char rely)
{
    ...
    event_initheader(&message.header, EVENT_MOUSEMOVE, EVENT_ADDR_BROADCAST);
    event_addmousemove(&message.header, relx, rely);
...
void mouse_notifypress(struct mouse_interface *interface, unsigned int button)
{
    ...
    event_initheader(&message.header, EVENT_MOUSEPRESS, EVENT_ADDR_BROADCAST);
    event_addmousepress(&message.header, button);
    kernel_multicast(&event.states, &message.header);
    kernel_multicast(&interface->event.states, &message.header);
...
void mouse_notifyrelease(struct mouse_interface *interface, unsigned int button)
{
    ...
    event_initheader(&message.header, EVENT_MOUSERELEASE, EVENT_ADDR_BROADCAST);
    event_addmouserelease(&message.header, button);
    kernel_multicast(&event.states, &message.header);
    kernel_multicast(&interface->event.states, &message.header);
