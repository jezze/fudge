...
static void blittext(struct render_display *display, struct font *font, unsigned int color, char *string, unsigned int length, int x, int y, int x0, int x1, int y0)
{
    ...
    if (util_intersects(y, y0, y0 + font->lineheight))
    {
        ...
        unsigned int line = (y - y0) % font->lineheight;
        unsigned int i;
...
static void paintbutton(struct render_display *display, struct widget *widget, int y, int x0, int x1)
{
    ...
    blittext(display, &fonts[0], 0xFFFFFFFF, button->label, cstring_length(button->label), widget->position.x + 32, y, x0 + 32, x1 - 32, widget->position.y + (widget->size.h / 2) - (16 / 2));
