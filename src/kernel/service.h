...
struct service_protocol
{
    ...
    unsigned int (*open)(struct service_backend *backend, struct list_item *link, struct service_state *state);
    unsigned int (*close)(struct service_backend *backend, struct list_item *link, struct service_state *state);
    unsigned int (*read)(struct service_backend *backend, struct list_item *link, struct service_state *state, unsigned int count, void *buffer);
    unsigned int (*write)(struct service_backend *backend, struct list_item *link, struct service_state *state, unsigned int count, void *buffer);
    unsigned int (*seek)(struct service_backend *backend, struct service_state *state, unsigned int offset);
    unsigned int (*scan)(struct service_backend *backend, struct service_state *state, unsigned int index);
    unsigned long (*map)(struct service_backend *backend, unsigned int id, struct binary_node *node);
