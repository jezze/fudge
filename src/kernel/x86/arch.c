...
static unsigned int spawn(unsigned int itask, void *stack)
{
    ...
    struct {void *caller; unsigned int ichannel; unsigned int id; unsigned int address;} *args = stack;
    ...
    if (args->ichannel && args->id)
    {
        ...
        if (ntask)
        {
            ...
            return kernel_loadtask(ntask, 0, ARCH_TASKSTACKVIRTUAL, args->ichannel, args->id, args->address);
...
void arch_setup1(void)
{
    ...
    arch_map(2, ARCH_MMUKERNELADDRESS, ARCH_MMUKERNELADDRESS, 0x00400000);
    arch_map(3, ARCH_MMUTASKADDRESS, ARCH_MMUTASKADDRESS, ARCH_MMUTASKSIZE * KERNEL_TASKS);
    arch_map(4, ARCH_MAILBOXADDRESS, ARCH_MAILBOXADDRESS, ARCH_MAILBOXSIZE * KERNEL_MAILBOXES);
...
void arch_setup2(unsigned int address)
{
    ...
    if (ntask)
    {
        ...
        initmap(ntask);
        kernel_loadtask(ntask, 0, ARCH_TASKSTACKVIRTUAL, 0, 0, address);
