...
static void reply(unsigned int source, unsigned short type, char *name, void *rddata, void *buffer)
{
    ...
    if (type == 1)
    {
        ...
        channel_send_fmt6(source, EVENT_QUERYRESPONSE, "type\\01\\0name\\0%w\\0data\\0%c.%c.%c.%c\\0", fullname, &fullnamelength, &addr[0], &addr[1], &addr[2], &addr[3]);
    ...
    else if (type == 5)
    {
        ...
        channel_send_fmt4(source, EVENT_QUERYRESPONSE, "type\\05\\0name\\0%w\\0data\\0%w\\0", fullname, &fullnamelength, alias, &aliaslength);
