...
static void print_icmp(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "ICMP:\n");
    channel_sendfmt1(EVENT_DATA, "  Type: 0x%H2c\n", &header->type[0]);
    channel_sendfmt1(EVENT_DATA, "  Code: 0x%H2c\n", &header->code[0]);
...
static void print_tcp(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "TCP:\n");
    channel_sendfmt1(EVENT_DATA, "  Source Port: %h\n", &sp);
    channel_sendfmt1(EVENT_DATA, "  Target Port: %h\n", &tp);
    channel_sendfmt4(EVENT_DATA, "  Sequence: 0x%H2c%H2c%H2c%H2c\n", &header->seq[0], &header->seq[1], &header->seq[2], &header->seq[3]);
    channel_sendfmt4(EVENT_DATA, "  Ack: 0x%H2c%H2c%H2c%H2c\n", &header->ack[0], &header->ack[1], &header->ack[2], &header->ack[3]);
    channel_sendfmt2(EVENT_DATA, "  Flags: 0x%H2c%H2c\n", &header->flags[0], &header->flags[1]);
    channel_sendfmt2(EVENT_DATA, "  Window: 0x%H2c%H2c\n", &header->window[0], &header->window[1]);
...
static void print_udp(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "UDP:\n");
    channel_sendfmt1(EVENT_DATA, "  Source Port: %h\n", &sp);
    channel_sendfmt1(EVENT_DATA, "  Target Port: %h\n", &tp);
    channel_sendfmt1(EVENT_DATA, "  Length: %h\n", &length);
...
static void print_arp(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "ARP:\n");
    channel_sendfmt2(EVENT_DATA, "  Hardware Type: 0x%H2c%H2c\n", &header->htype[0], &header->htype[1]);
    channel_sendfmt2(EVENT_DATA, "  Protocol Type: 0x%H2c%H2c\n", &header->ptype[0], &header->ptype[1]);
    channel_sendfmt1(EVENT_DATA, "  Hardware Length: 0x%H2c\n", &header->hlength[0]);
    channel_sendfmt1(EVENT_DATA, "  Protocol Length: 0x%H2c\n", &header->plength[0]);
    channel_sendfmt2(EVENT_DATA, "  Operation: 0x%H2c%H2c\n", &header->operation[0], &header->operation[1]);
...
static void print_ipv4(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "IPv4:\n");
    channel_sendfmt1(EVENT_DATA, "  Protocol: 0x%H2c\n", &header->protocol[0]);
    channel_sendfmt4(EVENT_DATA, "  Source Addr (IP): %c.%c.%c.%c\n", &header->sip[0], &header->sip[1], &header->sip[2], &header->sip[3]);
    channel_sendfmt4(EVENT_DATA, "  Target Addr (IP): %c.%c.%c.%c\n", &header->tip[0], &header->tip[1], &header->tip[2], &header->tip[3]);
...
static void print_ipv6(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "IPv6:\n");
    channel_sendfmt1(EVENT_DATA, "  Next: 0x%H2c\n", &header->next[0]);
    channel_sendfmt1(EVENT_DATA, "  Hop: 0x%H2c\n", &header->hop[0]);
    channel_sendfmt0(EVENT_DATA, "  Source Addr (IP): ");
    channel_sendfmt8(EVENT_DATA, "%H2c%H2c:%H2c%H2c:%H2c%H2c:%H2c%H2c:", &header->sip[0], &header->sip[1], &header->sip[2], &header->sip[3], &header->sip[4], &header->sip[5], &header->sip[6], &header->sip[7]);
    channel_sendfmt8(EVENT_DATA, "%H2c%H2c:%H2c%H2c:%H2c%H2c:%H2c%H2c\n", &header->sip[8], &header->sip[9], &header->sip[10], &header->sip[11], &header->sip[12], &header->sip[13], &header->sip[14], &header->sip[15]);
    channel_sendfmt0(EVENT_DATA, "  Target Addr (IP): ");
    channel_sendfmt8(EVENT_DATA, "%H2c%H2c:%H2c%H2c:%H2c%H2c:%H2c%H2c:", &header->tip[0], &header->tip[1], &header->tip[2], &header->tip[3], &header->tip[4], &header->tip[5], &header->tip[6], &header->tip[7]);
...
static void print_ethernet(unsigned int source, void *buffer)
{
    ...
    channel_sendfmt0(EVENT_DATA, "Ethernet:\n");
    channel_sendfmt6(EVENT_DATA, "  Source Addr (MAC): %H2c:%H2c:%H2c:%H2c:%H2c:%H2c\n", &header->sha[0], &header->sha[1], &header->sha[2], &header->sha[3], &header->sha[4], &header->sha[5]);
    channel_sendfmt6(EVENT_DATA, "  Target Addr (MAC): %H2c:%H2c:%H2c:%H2c:%H2c:%H2c\n", &header->tha[0], &header->tha[1], &header->tha[2], &header->tha[3], &header->tha[4], &header->tha[5]);
    channel_sendfmt2(EVENT_DATA, "  Type: 0x%H2c%H2c\n", &header->type[0], &header->type[1]);
