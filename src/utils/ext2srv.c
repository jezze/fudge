...
static unsigned int sendblockrequest(void *buffer, unsigned int count, unsigned int sector, unsigned int blocksize)
{
...
static void readsuperblock(void)
{
    ...
    sendblockrequest(data, 1024, 1, 1024);
...
static void readblockgroup(struct ext2_blockgroup *bg, unsigned int start, unsigned int index)
{
    ...
    sendblockrequest(data, EXT2_MAXBLOCKSIZE, sector, blocksize);
...
static unsigned int getindirect(unsigned int sector, unsigned int index)
{
    ...
    if (sector)
    {
        ...
        sendblockrequest(table, blocksize, sector, blocksize);
...
static unsigned int matchentry(struct ext2_node *node, char *name, unsigned int length)
{
    ...
    while (offset < node->sizeLow)
    {
        ...
        sendblockrequest(block, EXT2_MAXBLOCKSIZE, sector, blocksize);
