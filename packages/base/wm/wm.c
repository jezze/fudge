...
static void enable(unsigned int xres, unsigned int yres, unsigned int bpp)
{
    ...
    call_open(3, 17, "/dev/video/1/xres");
    call_write(3, 0, 4, &xres);
    call_open(3, 17, "/dev/video/1/yres");
    call_write(3, 0, 4, &yres);
    call_open(3, 16, "/dev/video/1/bpp");
    call_write(3, 0, 4, &bpp);
    call_open(3, 19, "/dev/video/1/enable");
...
void main()
{
    ...
    unsigned int id = call_open(4, 17, "/dev/video/1/data");
