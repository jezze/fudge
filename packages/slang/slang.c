    ...
    for (pindex = 0; pindex < expression->count; pindex++)
    {
        ...
        for (cindex = 0; cindex < pipe->count; cindex++)
        {
            ...
            if (command->in0.path.count)
            {
                ...
                open_path(CALL_I0, command->in0.path.count, state->buffer + command->in0.path.index);
            ...
            else if (command->in0.data.count)
            {
                ...
                call_write(CALL_L5, 0, command->in0.data.count, state->buffer + command->in0.data.index);
            ...
            if (command->in1.path.count)
            {
                ...
                open_path(CALL_I1, command->in1.path.count, state->buffer + command->in1.path.index);
            ...
            else if (command->in1.data.count)
            {
                ...
                call_write(CALL_L5, 0, command->in1.data.count, state->buffer + command->in1.data.index);

            }

            if (command->out0.path.count)
                open_path(CALL_O0, command->out0.path.count, state->buffer + command->out0.path.index);

            call_open(CALL_DW, CALL_DR, 4, "bin/");
            open_path(CALL_E0, command->binary.count, state->buffer + command->binary.index);
