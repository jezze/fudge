...
void kernel_setup(struct runtime_task *task, unsigned int ramdiskc, void **ramdiskv)
{
    ...
    runtime_init_mount(&task->mounts[1], root, root->rootid, root, root->rootid, 1, "/");
    runtime_init_mount(&task->mounts[2], root, root->walk(root, root->rootid, 8, "ramdisk/"), ramdisk, ramdisk->rootid, 9, "/ramdisk/");
