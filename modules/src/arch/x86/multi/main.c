...
static unsigned int spawn(struct runtime_task *task, void *stack)
{
    ...
    ntask->directory = (struct mmu_directory *)(MMU_ADDRESS_DIRECTORIES + MMU_PAGE_SIZE * index);
    ntask->ktable = (struct mmu_table *)(MMU_ADDRESS_KTABLES + MMU_PAGE_SIZE * index);
    ntask->utable = (struct mmu_table *)(MMU_ADDRESS_UTABLES + MMU_PAGE_SIZE * index);

    memory_clear(ntask->directory, sizeof (struct mmu_directory));
    memory_clear(ntask->ktable, sizeof (struct mmu_table));
    memory_clear(ntask->utable, sizeof (struct mmu_table));

    mmu_map_memory(ntask->directory, ntask->ktable, ARCH_KERNEL_BASE, ARCH_KERNEL_BASE, ARCH_KERNEL_SIZE, MMU_TABLE_FLAG_PRESENT | MMU_TABLE_FLAG_WRITEABLE, MMU_PAGE_FLAG_PRESENT | MMU_PAGE_FLAG_WRITEABLE);
    mmu_map_memory(ntask->directory, ntask->utable, RUNTIME_TASK_PADDRESS_BASE + index * RUNTIME_TASK_ADDRESS_SIZE, RUNTIME_TASK_VADDRESS_BASE, RUNTIME_TASK_ADDRESS_SIZE, MMU_TABLE_FLAG_PRESENT | MMU_TABLE_FLAG_WRITEABLE | MMU_TABLE_FLAG_USERMODE, MMU_PAGE_FLAG_PRESENT | MMU_PAGE_FLAG_WRITEABLE | MMU_PAGE_FLAG_USERMODE);
